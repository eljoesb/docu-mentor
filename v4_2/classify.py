"""
classify.py — Clasificacion ciega por eje (retrieval / generation / utilidad).

Tres pasadas independientes, cada una muestra solo la info relevante:
  --axis retrieval   → muestra query + chunk         → pide b/t/m
  --axis generation  → muestra chunk + llm_response  → pide f/x/i (fiel/mixto/inventado)
  --axis utility     → muestra query + llm_response  → pide u/p/d (util/parcial/danino)

Uso:
    python v3/classify.py pipeline_results.jsonl --axis retrieval
    python v3/classify.py pipeline_results.jsonl --axis generation
    python v3/classify.py pipeline_results.jsonl --axis utility

Crash-safe: guarda CSV despues de cada clasificacion.
Retoma desde donde quedo si el CSV existe.
"""
...
AXES = {
    ...
    "generation": {
        "labels": {"f": "fiel", "x": "mixto", "i": "inventado"},
        "prompt": "f / x / i ? ",
        "criteria": (
...
def main():
    ...
    parser.add_argument("-o", "--output", help="CSV de salida (default: labels_<axis>.csv)")
    ...
    if args.output:
        ...
        csv_path = jsonl_path.parent / f"labels_{axis}.csv"
    ...
    print(f"Corre count.py {csv_path} para ver conteos.")
